...
def prime(n):
	...
	if prime_factors == []:				# checks if the number is already prime itself
		return [n]
	if n % 2 != 0:						# because we check only that prime numbers which are under sqrt(n), the last n isn's compared to list_prime (it's larger), so we check if it is prime or not, and if yes, add to prime factors list
